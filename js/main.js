...
function Producto(compra, cantidad) {
  this.compra = compra.toLowerCase(),  //: por =
...
function mergeHTML (){
  ...
  for (i=0; i<productos.length; i++){   //;
    html += productos[i].toHTML();   //coma[i,]
  }
  return html;
}
...
addCompra.onclick = function() {
  ...
  var product  = new Producto (compra, cantidad); //nombre de clase estaba como compra
